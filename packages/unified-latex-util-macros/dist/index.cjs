...
function createMatchers() {
	return {
		isHash: (node) => _unified_latex_unified_latex_util_match.match.string(node, "#"),
		isNumber: (node) => _unified_latex_unified_latex_util_match.match.string(node) && 0 < +node.content.charAt(0),
		splitNumber: (node) => {
...
function parseMacroSubstitutions(ast) {
	...
	return _unified_latex_unified_latex_util_pegjs.MacroSubstitutionPegParser.parse(ast, createMatchers());
}
//#endregion
...
function getNewcommandNamedArgs(node) {
	...
	return (0, _unified_latex_unified_latex_util_arguments.getNamedArgsContent)(node, node.args.length === NEWCOMMAND_ARGUMENTS_BEAMER.length ? NEWCOMMAND_ARGUMENTS_BEAMER : NEWCOMMAND_ARGUMENTS_REG);
}
/**
...
function newcommandMacroToSpec(node) {
	if (LATEX_NEWCOMMAND.has(node.content)) {
		...
		let numArgsForSig = +(0, _unified_latex_unified_latex_util_print_raw.printRaw)(namedArgs.numArgs);
		let sigOptionalArg = [];
		if (namedArgs.default != null) {
			numArgsForSig--;
			sigOptionalArg = [`O{${(0, _unified_latex_unified_latex_util_print_raw.printRaw)(namedArgs.default)}}`];
		}
	...
	if (XPARSE_NEWCOMMAND.has(node.content)) {
		...
		return (0, _unified_latex_unified_latex_util_print_raw.printRaw)(node.args[1]?.content).trim();
	}
	return "";
}
/**
...
function newcommandMacroToName(node) {
	if (LATEX_NEWCOMMAND.has(node.content)) {
		...
		return normalizeCommandName((0, _unified_latex_unified_latex_util_print_raw.printRaw)(definedName));
	}
	if (XPARSE_NEWCOMMAND.has(node.content)) {
		if (!node.args?.length) return "";
		...
		return normalizeCommandName((0, _unified_latex_unified_latex_util_print_raw.printRaw)(node.args[0].content));
	}
	return "";
}
/**
...
function createMacroExpander(substitution) {
	...
	return (macro) => {
		...
		(0, _unified_latex_unified_latex_util_replace.replaceNode)(retTree, (node) => {
...
function cachedParse(source) {
	...
	const parsed = (0, _unified_latex_unified_latex_util_parse.parseMinimal)(source).content;
...
function expandCachedSubstitutions(expanderArgs) {
	...
	for (const expanderArg of expanderArgs) {
		...
		(0, _unified_latex_unified_latex_util_replace.replaceNode)(expanderArg.content, (node) => {
			const hashNumOrNode = node;
			if (hashNumOrNode.type !== "hash_number") return;
			didExpand = true;
...
var newcommandMatcher = _unified_latex_unified_latex_util_match.match.createMacroMatcher([...LATEX_NEWCOMMAND, ...XPARSE_NEWCOMMAND]);
/**
...
function expandMacros(tree, macros) {
	...
	(0, _unified_latex_unified_latex_util_replace.replaceNode)(tree, (node) => {
		if (!_unified_latex_unified_latex_util_match.match.anyMacro(node)) return;
