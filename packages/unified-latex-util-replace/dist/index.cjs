...
function replaceNode(ast, visitor) {
	(0, _unified_latex_unified_latex_util_visit.visit)(ast, { leave: (node, info) => {
...
function lastSignificantNodeIndex(nodes, parbreaksAreInsignificant) {
	for (let i = nodes.length - 1; i >= 0; i--) {
		const node = nodes[i];
		if (_unified_latex_unified_latex_util_match.match.whitespace(node) || _unified_latex_unified_latex_util_match.match.comment(node) || parbreaksAreInsignificant && _unified_latex_unified_latex_util_match.match.parbreak(node)) continue;
		return i;
	}
}
/**
...
function isSpaceLike(node) {
	return _unified_latex_unified_latex_util_match.match.whitespace(node) || _unified_latex_unified_latex_util_match.match.comment(node) && Boolean(node.leadingWhitespace);
}
/**
...
function joinWithoutExcessWhitespace(head, tail) {
	...
	if (_unified_latex_unified_latex_util_match.match.whitespace(headEnd) && _unified_latex_unified_latex_util_match.match.whitespace(tailStart)) {
		head.push(...tail.slice(1));
		return;
	}
	if (!isSpaceLike(headEnd) || !isSpaceLike(tailStart)) {
		if (_unified_latex_unified_latex_util_match.match.whitespace(headEnd) && _unified_latex_unified_latex_util_match.match.comment(tailStart)) {
			const comment = {
				type: "comment",
			...
			tail = tail.slice(1);
			(0, _unified_latex_unified_latex_util_trim.trimStart)(tail);
			head.pop();
	...
	if (_unified_latex_unified_latex_util_match.match.comment(headEnd) && _unified_latex_unified_latex_util_match.match.comment(tailStart)) {
	...
	let comment = _unified_latex_unified_latex_util_match.match.comment(headEnd) ? headEnd : tailStart;
	if (!_unified_latex_unified_latex_util_match.match.comment(comment)) throw new Error(`Expected a comment but found ${JSON.stringify(comment)}`);
...
function wrapSignificantContent(content, wrapper) {
	...
	for (let i = 0; i < content.length; i++) {
		if (_unified_latex_unified_latex_util_match.match.whitespace(content[i]) || _unified_latex_unified_latex_util_match.match.comment(content[i])) {
			hoistUntil = i + 1;
			continue;
		}
		break;
	...
	for (let j = content.length - 1; j >= 0; j--) {
		if (_unified_latex_unified_latex_util_match.match.whitespace(content[j]) || _unified_latex_unified_latex_util_match.match.comment(content[j])) {
			hoistAfter = j;
			continue;
		}
		break;
...
function replaceStreamingCommandInArray(nodes, isStreamingCommand, replacer) {
	while (nodes.length > 0 && isStreamingCommand(nodes[nodes.length - 1])) {
		nodes.pop();
		(0, _unified_latex_unified_latex_util_trim.trimEnd)(nodes);
	}
...
function replaceStreamingCommand(ast, isStreamingCommand, replacer, options) {
	...
	if (_unified_latex_unified_latex_util_match.match.group(ast)) processedContent = replaceStreamingCommandInGroup(ast, isStreamingCommand, replacer);
	if (Array.isArray(ast)) {
		const nodes = ast;
		let scanIndex = nodes.length;
		let sliceIndex = scanIndex;
		while (scanIndex > 0 && (isStreamingCommand(nodes[scanIndex - 1]) || _unified_latex_unified_latex_util_match.match.whitespace(nodes[scanIndex - 1]))) {
			scanIndex--;
			if (isStreamingCommand(nodes[scanIndex])) sliceIndex = scanIndex;
		}
		if (sliceIndex !== nodes.length) nodes.splice(sliceIndex);
		const macroThatBreaks = _unified_latex_unified_latex_util_match.match.createMacroMatcher(macrosThatBreakPars);
		const envThatDoesntBreak = _unified_latex_unified_latex_util_match.match.createEnvironmentMatcher(environmentsThatDontBreakPars);
		const isPar = (node) => _unified_latex_unified_latex_util_match.match.parbreak(node) || _unified_latex_unified_latex_util_match.match.macro(node, "par") || macroThatBreaks(node) || _unified_latex_unified_latex_util_match.match.environment(node) && !envThatDoesntBreak(node) || node.type === "displaymath";
		const splitByPar = (0, _unified_latex_unified_latex_util_split.splitOnCondition)(nodes, isPar);
		splitByPar.separators = splitByPar.separators.map((sep) => _unified_latex_unified_latex_util_match.match.macro(sep, "par") ? { type: "parbreak" } : sep);
		const replacers = [];
		...
		if (segments.length > 1) segments.forEach((segment, i) => {
			...
			else if (i === segments.length - 1) (0, _unified_latex_unified_latex_util_trim.trimStart)(segment);
			else (0, _unified_latex_unified_latex_util_trim.trim)(segment);
		});
		processedContent = (0, _unified_latex_unified_latex_util_split.unsplitOnMacro)({
			segments,
...
var unifiedLatexReplaceStreamingCommands = function unifiedLatexReplaceStreamingCommands(options) {
	...
	const isReplaceable = _unified_latex_unified_latex_util_match.match.createMacroMatcher(replacers);
	return (tree) => {
		...
		}, { test: _unified_latex_unified_latex_util_match.match.group });
