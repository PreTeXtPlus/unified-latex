...
function parseTexGlue(source) {
	try {
		return _unified_latex_unified_latex_util_pegjs.GluePegParser.parse(source);
	} catch {}
	return null;
}
//#endregion
...
function findGlue(nodes, startIndex) {
	...
	for (let i = startIndex; i < nodes.length; i++) {
		const node = nodes[i];
		if (_unified_latex_unified_latex_util_match.match.whitespace(node) || _unified_latex_unified_latex_util_match.match.comment(node)) continue;
		if (!_unified_latex_unified_latex_util_match.match.anyString(node)) break;
