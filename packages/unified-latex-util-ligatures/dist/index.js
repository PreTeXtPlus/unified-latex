...
function hashNodes(nodes) {
  return nodes.map((node) => match.macro(node) ? `\\${node.content}` : node.content).join(" ");
}
...
function createMatchers() {
  return {
    ...
    isSplitable: (node) => match.anyString(node) && node.content.length > 1,
    split: (node) => [
