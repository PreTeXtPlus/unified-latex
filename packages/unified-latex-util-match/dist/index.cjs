...
function createEnvironmentMatcher(macros) {
	...
	return function matchAgainstEnvironments(node) {
		...
		if (!environmentsHash[(0, _unified_latex_unified_latex_util_print_raw.printRaw)(node.env)]) return false;
		return true;
	};
}
/**
...
var match = {
	...
	environment(node, envName) {
		if (node == null) return false;
		return (node.type === "environment" || node.type === "mathenv") && (envName == null || (0, _unified_latex_unified_latex_util_print_raw.printRaw)(node.env) === envName);
	},
	anyEnvironment(node) {
