...
import { unifiedLatexReplaceStreamingCommands, replaceNode } from "@unified-latex/unified-latex-util-replace";
...
import { xcolorMacroToHex, colorToTextcolorMacro } from "@unified-latex/unified-latex-ctan/package/xcolor";
  ...
  return (tree) => {
    ...
    replaceNode(tree, (node, info) => {
      ...
      if (isReplaceableEnvironment(node)) {
        return environmentReplacements$1[printRaw(node.env)](node, info);
      }
    });
    replaceNode(tree, (node, info) => {
      if (info.context.hasMathModeAncestor) {
...
const _processor = processLatexViaUnified().use(unifiedLatexToHast).use(rehypeStringify);
