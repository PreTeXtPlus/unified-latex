...
let _unified_latex_unified_latex_util_packages = require("@unified-latex/unified-latex-util-packages");
//#region rules/unified-latex-lint-obsolete-packages/index.ts
var OBSOLETE = {
...
`;
var unifiedLatexLintObsoletePackages = require_lib.lintRule({ origin: "unified-latex-lint:obsolete-packages" }, (tree, file, options) => {
	for (const packageStr of (0, _unified_latex_unified_latex_util_packages.listPackages)(tree)) {
		const packageName = packageStr.content;
		if (packageName in OBSOLETE) file.message(`Inclusion of obsolete package \`${packageName}\`. Suggestion: ${OBSOLETE[packageName]}.`, packageStr);
	}
});
//#endregion
