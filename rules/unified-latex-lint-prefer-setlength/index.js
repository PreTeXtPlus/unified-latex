...
var isLengthMacro = match.createMacroMatcher([
	"abovecaptionskip",
	"arraycolsep",
	"arrayrulewidth",
	"belowcaptionskip",
...
`;
var unifiedLatexLintPreferSetlength = lintRule({ origin: "unified-latex-lint:prefer-setlength" }, (tree, file, options) => {
	visit(tree, (node, info) => {
		...
		file.message(`TeX-style assignment to length \`${printRaw(node)}\`; prefer LaTeX \`\\setlength{${printRaw(node)}}{...}\``, node);
		if (options?.fix) {
			const glue = extractFormattedGlue(containingArray, equalsIndex + 1);
			if (!glue) {
				console.warn("Expected to find glue following `=` but couldn't");
				return;
			}
			const numReplacements = glue.span.end - info.index + 1;
			containingArray.splice(info.index, numReplacements, ...[m("setlength", [arg(node), arg(glue.glue)]), ...glue.trailingStrings]);
			return info.index + 1;
		}
	}, { test: isLengthMacro });
});
//#endregion
export { DESCRIPTION, unifiedLatexLintPreferSetlength };
