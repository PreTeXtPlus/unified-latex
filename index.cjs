...
//#endregion
let prettier_standalone_js = require("prettier/standalone.js");
prettier_standalone_js = __toESM(prettier_standalone_js, 1);
let _unified_latex_unified_latex_prettier = require("@unified-latex/unified-latex-prettier");
...
var unifiedLatexStringCompiler = function unifiedLatexStringCompiler(options) {
	...
	const prettyPrinter = (ast) => {
		let formatted = prettier_standalone_js.default.format("_", {
			useTabs,
			printWidth,
			...
			plugins: [{
				...
				printers: { "latex-ast": { print: _unified_latex_unified_latex_prettier.printLatexAst } }
			}],
			...options || {}
		});
	...
	Object.assign(this, { Compiler: (ast) => {
		if (!pretty) return (0, _unified_latex_unified_latex_util_print_raw.printRaw)(ast);
