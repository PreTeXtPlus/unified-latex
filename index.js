...
import { decorateArrayForPegjs, MacroSubstitutionPegParser } from "@unified-latex/unified-latex-util-pegjs";
(function() {
...
function getNewcommandNamedArgs(node) {
  ...
  return getNamedArgsContent(node, argNames);
}
function newcommandMacroToSpec(node) {
  ...
  if (XPARSE_NEWCOMMAND.has(node.content)) {
    ...
    const macroSpec = printRaw((_b = node.args[1]) == null ? void 0 : _b.content);
