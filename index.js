...
import { LigaturesPegParser, decorateArrayForPegjs } from "@unified-latex/unified-latex-util-pegjs";
...
function ligatureToUnicode(nodes) {
	...
	if (nodes.length === 1 && match.macro(nodes[0], " ") && nodes[0].escapeToken == null) return makeString(" ");
	...
	return {
		...
		isRecognized: (nodes, whitespaceAllowed = false) => {
			...
			if (nodes.length === 2 && match.macro(nodes[0])) {
				const arg = nodes[1];
				if (match.group(arg) && arg.content.length === 1) nodesToTest[1] = arg.content[0];
...
*/
//#endregion
export { createMatchers, expandUnicodeLigatures, parseLigatures };
